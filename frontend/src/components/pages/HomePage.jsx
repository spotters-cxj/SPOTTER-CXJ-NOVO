          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {photos.length > 0 ? photos.map((photo) => (
              <div key={photo.photo_id} className="photo-card group">
                <img src={photo.url.startsWith('/api') ? `${process.env.REACT_APP_BACKEND_URL}${photo.url}` : photo.url} alt={photo.description} />
                <div className="photo-overlay">
                  ...
                  <p className="text-sky-400 text-xs mt-2">Por {photo.author_name}</p>
                </div>
              </div>
            )) : (
